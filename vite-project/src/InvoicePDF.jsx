...
const InvoicePDF = ({ formData }) => {
  ...
  // Check if any EstimateDetails field has meaningful data to conditionally render the section
  const hasEstimateDetails =
    (EstimateDetails.mmr && EstimateDetails.mmr.toString().trim() !== '') ||
    (EstimateDetails.profit && Number(EstimateDetails.profit) !== 0) ||
    (EstimateDetails.transport && EstimateDetails.transport.toString().trim() !== '') ||
    (EstimateDetails.carfaxStatus && EstimateDetails.carfaxStatus.toString().trim() !== '') ||
    (EstimateDetails.repair && EstimateDetails.repair.toString().trim() !== '') ||
    (EstimateDetails.fees && EstimateDetails.fees.toString().trim() !== '') ||
    (EstimateDetails.maxBid && EstimateDetails.maxBid.toString().trim() !== '') ||
    (EstimateDetails.autocheckStatus && EstimateDetails.autocheckStatus.toString().trim() !== '');
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        {/* Conditionally render Estimate Details */}
        {hasEstimateDetails && (
          <View style={styles.section}>
        ...
        {(TransportDetails.transporterName ||
          ...
          <View style={styles.section}>
            ...
            <View style={styles.row}>
              {TransportDetails.transporterName && <Text>Transporter: {TransportDetails.transporterName}</Text>}
              {TransportDetails.transporterPhone && <Text>Phone: {TransportDetails.transporterPhone}</Text>}
              {TransportDetails.cost > 0 && <Text>Cost: ${Number(TransportDetails.cost).toFixed(2)}</Text>}
            </View>

            <View style={styles.row}>
              {TransportDetails.pickupDate && <Text>Pickup: {new Date(TransportDetails.pickupDate).toLocaleDateString()}</Text>}
              {TransportDetails.deliveryDate && <Text>Delivery: {new Date(TransportDetails.deliveryDate).toLocaleDateString()}</Text>}
            </View>

            {TransportDetails.transporterNotes && <Text style={styles.notes}>Notes: {TransportDetails.transporterNotes}</Text>}
          </View>
        )}
        ...
        {PartsDetails.parts?.some(
          ...
          <View style={styles.section}>
            ...
            {PartsDetails.partsNotes && <Text style={styles.notes}>Notes: {PartsDetails.partsNotes}</Text>}
          </View>
        )}
        ...
        {MechanicDetails.mechanicServices?.some(
          ...
          <View style={styles.section}>
            ...
            {MechanicDetails.mechanicNotes && <Text style={styles.notes}>Notes: {MechanicDetails.mechanicNotes}</Text>}
          </View>
        )}
        ...
        {BodyshopDetails.bodyshopServices?.some(
          ...
          <View style={styles.section}>
            ...
            {BodyshopDetails.bodyshopNotes && <Text style={styles.notes}>Notes: {BodyshopDetails.bodyshopNotes}</Text>}
          </View>
        )}
        ...
        {MiscellaniousDetails.miscServices?.some(
          ...
          <View style={styles.section}>
            ...
            {MiscellaniousDetails.miscNotes && <Text style={styles.notes}>Notes: {MiscellaniousDetails.miscNotes}</Text>}
          </View>
        )}
        ...
        {(saleDetails.saleType ||
          ...
          <View style={styles.section}>
            ...
            {(saleDetails.saleDate || saleDetails.saleAmount || saleDetails.sellerFees || InvoiceDetails.grossProfit) && (
              <View style={styles.row}>
                {saleDetails.saleDate && <Text>Sale Date: {new Date(saleDetails.saleDate).toLocaleDateString()}</Text>}
                {saleDetails.saleAmount > 0 && <Text>Sale Amount: ${Number(saleDetails.saleAmount).toFixed(2)}</Text>}
                {saleDetails.sellerFees > 0 && <Text>Fees: ${Number(saleDetails.sellerFees).toFixed(2)}</Text>}
                {InvoiceDetails.grossProfit > 0 && <Text>Profit: ${Number(InvoiceDetails.grossProfit).toFixed(2)}</Text>}
              </View>
            )}
            ...
            {(saleDetails.soldTo || saleDetails.salesmanName || saleDetails.commission > 0 || InvoiceDetails.netProfit > 0) && (
              <View style={styles.row}>
                ...
                {saleDetails.commission > 0 && <Text>Commission: ${Number(saleDetails.commission).toFixed(2)}</Text>}
                {InvoiceDetails.netProfit > 0 && <Text>Net Profit: ${Number(InvoiceDetails.netProfit).toFixed(2)}</Text>}
              </View>
            )}

            {saleDetails.saleNotes && <Text style={styles.notes}>Notes: {saleDetails.saleNotes}</Text>}
          </View>
        )}
